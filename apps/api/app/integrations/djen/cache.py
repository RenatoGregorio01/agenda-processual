...
CACHE_PREFIX = "djen:v1:proc:"
LOCK_PREFIX = "djen:v1:lock:"
RATE_KEY = "djen:rl:global"
...
async def set_cached(redis: Redis, digitos: str, payload: dict[str, Any], *, empty: bool) -> None:
    ...
    ttl = settings.djen_empty_ttl_seconds if empty else settings.djen_cache_ttl_seconds
...
async def acquire_lock(redis: Redis, digitos: str) -> bool:
    ...
    acquired = await redis.set(
        ...
        "1",
        nx=True,
        ex=settings.djen_lock_ttl_seconds,
    )
...
async def allow_request(redis: Redis) -> bool:
    ...
    return current <= settings.djen_rate_limit_per_minute
